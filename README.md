...
(трамплин). Для UCONTEXT — сборка lcc, `freecontext`/`freecontext_e2k` (слабые ссылки
на оба имени; существование и сигнатура в glibc e2k не проверены — при
отсутствии символов аппаратные стеки ядра после `coro_destroy` утекут).
